...
# ===========================================================================
# Step 1: 定义测试集（可以是你的真实数据）
# ===========================================================================

def create_test_dataset():
    """创建测试集 - 这里用简单示例，实际中应该是你的真实数据"""
    ...
    test_cases = [
        ...
        # 测试用例 2: 直接回答
        ConversationExperience(
            messages=[{"role": "user", "content": "如何排序列表？"}],
            response="",
            feedback=CompositeFeedback(
                feedback_type=FeedbackType.SCORE,
                score=0.9,
...
def collect_failures_from_testset(adapter, prompt, test_cases, threshold=0.7):
    ...
    print(f"\n{'='*80}")
    print(f"在 {len(test_cases)} 个测试用例上评估...")
    print(f"{'='*80}")
        ...
        if score < expected_score:
            ...
            failure = ConversationExperience(
                ...
                feedback=CompositeFeedback(
                    ...
                    critique=f"得分 {score:.2f} < 期望 {expected_score:.2f}",
                    score=score
                )
            )
            failures.append(failure)
            print(f"  ❌ 用例 {i+1}: 失败 (得分 {score:.2f})")
        else:
            print(f"  ✓ 用例 {i+1}: 通过 (得分 {score:.2f})")
...
def fully_automatic_optimization(
    ...
    for iteration in range(max_iterations):
        ...
        print(f"\n[2/3] 发现 {len(failures)} 个失败案例，开始优化...")
...
def main():
    ...
    print(f"✓ 已保存到 optimized_prompt.txt")
